...
class Candidate:
    ...
    def creator(self):
        """Return the link to the user that created the audio"""
    ...
    def closePage(self):
        ...
        if (self.isWithdrawn() or self.isFSX()) and self.audioCount() <= 1:
        ...
        fifthDay = False if self.audioCount() > 1 else self.rulesOfFifthDay()
        ...
        if re.search(r"{{\s*FSC-closed-ignored.*}}", old_text):
    ...
    def fileName(self, alternative=True):
        ...
        then the first audio link in the page is used.
    ...
    def addToCategorizedFeaturedList(self, category):
        ...
        sounds. This is the full category.
    ...
    def addToCurrentMonth(self):
        """
        Adds the candidate to the list of featured sound this month
    ...
    def notifyNominator(self):
        ...
        try:
            self.commit(
                old_text, new_text, talk_page, "FSC promotion of [[%s]]" % fn_al
            )
    ...
    def moveToLog(self, reason=None):
        ...
        log_link = "Commons:Featured sound candidates/Log/%s %s" % (
            current_month,
            today.year,
        )
    ...
    def park(self):
        ...
        3. If featured or not move it from 'Commons:Featured sound candidates/candidate list'
...
class FSCandidate(Candidate):
    ...
    def __init__(self, page):
        ...
        self._listPageName = "Commons:Featured sound candidates/candidate list"

    def getResultString(self):
        if self.audioCount() > 1:
            return "\n\n{{FSC-results-ready-for-review|support=X|oppose=X|neutral=X|featured=no|category=|alternative=|sig=<small>'''Note: this candidate has several alternatives, thus if featured the alternative parameter needs to be specified.'''</small> /~~~~)}}"
        else:
            return (
                "\n\n{{FSC-results-ready-for-review|support=%d|oppose=%d|neutral=%d|featured=%s|category=|sig=~~~~}}"
...
class DelistCandidate(Candidate):
    ...
    def __init__(self, page):
        ...
        self._listPageName = "Commons:Featured sound candidates/removal"

    def getResultString(self):
        return (
            "\n\n{{FSC-delist-results-ready-for-review|delist=%d|keep=%d|neutral=%d|delisted=%s|sig=~~~~}}"
            % (self._pro, self._con, self._neu, "yes" if self.isPassed() else "no")
        )
    ...
    def removeFromFeaturedLists(self, results):
        ...
        # We skip checking the page with the 4 newest audios
    ...
    def removeAssessments(self):
        ...
        self.commit(old_text, new_text, audioPage, "Delisted")
    ...
    for template in templates:
        ...
        if title.startswith(candPrefix):
            ...
            if delist:
                ...
                candidates.append(FSCandidate(template))
        else:
            pass
...
VerifiedResultR = re.compile(
    r"""
                              {{\s*FSC-results-reviewed\s*\|        # Template start
                              ...
                              \s*category\s*=\s*([^|]*)             # A category if the audio was featured (5)
                              (?:\|\s*alternative\s*=\s*([^|]*))?   # For candidate with alternatives this specifies the winning audio (6)
                              .*}}                                  # END
                              """,
...
VerifiedDelistResultR = re.compile(
    r"{{\s*FSC-delist-results-reviewed\s*\|\s*delist\s*=\s*(\d+)\s*\|\s*keep\s*=\s*(\d+)\s*\|\s*neutral\s*=\s*(\d+)\s*\|\s*delisted\s*=\s*(\w+).*?}}",
    re.MULTILINE,
)

# Matches the entire line including newline so they can be stripped away
CountedTemplateR = re.compile(
    r"^.*{{\s*FSC-results-ready-for-review.*}}.*$\n?", re.MULTILINE
)
DelistCountedTemplateR = re.compile(
    r"^.*{{\s*FSC-delist-results-ready-for-review.*}}.*$\n?", re.MULTILINE
)
ReviewedTemplateR = re.compile(r"^.*{{\s*FSC-results-reviewed.*}}.*$\n?", re.MULTILINE)
DelistReviewedTemplateR = re.compile(
    r"^.*{{\s*FSC-delist-results-reviewed.*}}.*$\n?", re.MULTILINE
)
